...
def detect_tls_version_by_keyshare(data):
    ...
    try:
        ...
        handshake_len = (
            (handshake_data[1] << 16) | (handshake_data[2] << 8) | handshake_data[3]
        )
...
def generate_tls_alert(data):
    ...
    record_type, version_major, version_minor, record_length = struct.unpack(
        "!BBBH", data[:5]
    )
    alert_type = 0x15
    ...
    record_header = struct.pack(
        ">BHH", alert_type, (version_major << 8) | version_minor, len(alert_payload)
    )
...
def fake_udp_dns_query(query):
    ...
    for question in dns_query.question:
        if question.rdtype == dns.rdatatype.A:
            # A记录返回127.0.0.1
            a_record = dns.rrset.from_text(
                question.name, 3600, "IN", "A", "127.0.0.114"
            )
            response.answer.append(a_record)
        elif question.rdtype == dns.rdatatype.AAAA:
            # AAAA记录返回::1
            aaaa_record = dns.rrset.from_text(
                question.name, 3600, "IN", "AAAA", "::114"
            )
